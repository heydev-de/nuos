...
function ifc_command(object)
{
    if (object.getAttribute("data-ifc-confirm") && (! confirm(object.textContent + "?"))) return;
    try { (new Function(object.getAttribute("data-ifc-command"))).call(object); } catch (e) {};
};
                   ...
                   length)
{
    ...
    if (value.length > length) ifc_set(object, value.substring(0, length));
};
                   ...
                   format = false)
{
    ...
    //replace line/paragraph separators with line break
    value = value.replace(/[\u2028\u2029]/g, "\n");

    //replace tab with 4 spaces
    value = value.replace(/\t/g, "    ");

    //remove whitepace before line break
    value = value.replace(/[^\S\n]+\n/g, "\n");

    if (format)
    {
        //replace multiple spaces or a single line break with a space
        value = value.replace(/(\S)( +|\n)(\S)/g, "$1 $3");
