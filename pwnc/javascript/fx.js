...
function fx_scrollto(object)
{
    ...
    do
    {
        if (_object === fx_scroll_container)           break;
        if (fx_style(_object, "position") === "fixed") return;
    }
    while (_object = _object.parentElement);
    ...
    ["keydown", "mousedown", "touchstart", "wheel"].forEach((event) =>
    {
    ...
    var func = function(time)
    {
        ...
        var _x = fx_left(object) - fx_window_left;
        var _y = fx_top(object) - fx_window_top - fx_window_height / 5;

        count = ((Math.abs(x - _x) < 1) && (Math.abs(y - _y) < 1)) ? count + 1 : 0;
        if (count >= 5) { abort(); return; };
        x = _x; y = _y;

        if (start === null) start = time;
        var progress = 1 / duration * (time - start);

        fx_scroll_container.scrollTo(
            fx_window_left + x * progress,
            fx_window_top  + y * progress);

        if (progress >= 1) { abort(); return; };
        fx_animation_frame(func);
    };
                      ...
                      callback)
{
    ...
    fx_event_listen(object, "touchstart", function(e)
    {
        ...
        if (fx_touch2_x !== null)
        {
            ...
            _x1     = _x2 = fx_touch2_x;
            _y1     = _y2 = fx_touch2_y;
            d       = Math.sqrt(Math.pow(_x1 - x1, 2) + Math.pow(_y1 - y1, 2));
        };
    });
...
function _fx_event_raise(event, e)
{
    ...
    for (callback of fx_callback.slice())
    {
        if (callback.fx_event && (! callback.fx_event.includes(event))) continue;

        _event = callback.fx_pass_event_object ? e : event;
...
window.addEventListener("resize", (e) =>
{
    fx_update_window_position();
    fx_update_window_size();
    fx_event_raise("window_resize", e);
},
{ passive: true });

window.addEventListener("beforeunload", (e) =>
{
    _fx_event_raise("window_unload", e);
},
{ passive: true });
...
document.addEventListener("DOMContentLoaded", (e) =>
{
    ...
    fx_scroll_container.addEventListener("scroll", (e) =>
    {
        fx_update_window_position();
        fx_event_raise("window_scroll", e);
    },
    { passive: true });

    fx_update_window_position();
    fx_update_window_size();
    _fx_event_raise("document_load", e);
},
{ once: true });
...
{
    if (fx_ghost_buster_control) return;
    fx_update_mouse_position(e);
...
{
    _fx_event_raise(
        ((fx_mouse_key > 0) && ((e.button + 1) !== fx_mouse_key)) ?
        "mouseleave" : "mousedown", e);
},
{ passive: true });

document.addEventListener("mousemove", (e) =>
{
    if (! fx_ghost_buster_control) fx_update_mouse_position(e);
},
{ capture: true, passive: true });

document.addEventListener("mousemove", (e) =>
{
    fx_event_raise("mousemove", e);
},
{ passive: true });

document.addEventListener("mouseup", (e) =>
{
    _fx_event_raise("mouseup", e);
    fx_mouse_key = false;
},
{ passive: true });
...
document.addEventListener("touchstart", (e) =>
{
    ...
    while (object)
    {
        ...
        if (object.matches(":is(CODE:read-write, INPUT, LABEL, OPTION, SELECT, TEXTAREA)"))
        {
            fx_ghost_buster_zuul = true;
            break;
        };
...
document.addEventListener("keydown", (e) =>
{
    ...
    _fx_event_raise("keydown", e);
},
{ passive: true });

document.addEventListener("keypress", (e) =>
{
    _fx_event_raise("keypress", e);
},
{ passive: true });

document.addEventListener("keyup", (e) =>
{
    _fx_event_raise("keyup", e);
