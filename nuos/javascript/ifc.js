...
function ifc_highlight_init()
{
    ...
    for (i = 0, c = ifc_highlight_detect.length; i < c; i++)
    {
        if (! ifc_highlight_detect[i]) continue;

        for (_i = 0, _c = ifc_highlight_detect[i].length; _i < _c; _i++)
        {
            value = ifc_highlight_detect[i][_i][0].toString();
            key   = array.indexOf(value);
            if (key === -1) { key = __i++; array[key] = value; };
            ifc_highlight_detect[i][_i][4] = key;
        };
    };
};
